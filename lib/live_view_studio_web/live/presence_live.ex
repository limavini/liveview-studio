defmodule LiveViewStudioWeb.PresenceLive do
  # Read more: https://online.pragmaticstudio.com/courses/liveview-2ed-pro/steps/46
  ...
  def render(assigns) do
    ...
    <div id="presence">
      <h2>Who's here?</h2>
      <ul>
        <li :for={{user_id, meta} <- @presences} class="border p-1" id={user_id}>
          Username: <%= meta.username %>
          <br />
          Status: <%= if meta.is_playing,
            do: "Playing",
            else: "Not Playing" %>
        </li>
      </ul>
  ...
  # Diff is a map that contains "joins" and "leaves"
